...
- **The wizard's UI follows clack** (`cli/_prompt.py`): one connected rail (`┌ │ └`), answered steps redrawn as `◇ question` + value, `●`/`○` for pick-one vs `◼`/`◻` for pick-any, cyan = cursor, green = chosen. It degrades to numbered menus without a tty and to ASCII glyphs on a terminal that cannot encode the symbols. Every prompt is answered *before* the first write or delete, so Ctrl-C leaves nothing half-applied — preserve that when adding a step.
