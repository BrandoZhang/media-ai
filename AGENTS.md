...
- **The version lives in one place.** `__version__` in `src/media_ai/__init__.py`; `pyproject.toml` reads it via `dynamic = ["version"]`. The single exception is `DEFAULT_VERSION` in `install/install.sh` — a fallback *git ref*, not a package version — which `tests/test_version.py` pins to it. Releases go through **Actions → release** (`.github/workflows/release.yml`), which tests, bumps (`scripts/bump_version.py`), tags, builds and publishes; don't tag by hand, and don't reintroduce a static version in `pyproject.toml`. See `docs/DEVELOPMENT.md`.
