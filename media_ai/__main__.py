...
Groups: ``image``, ``video``, ``speech``, ``concat``, ``job``, ``capabilities``,
``usage``. Each group is also reachable directly; this umbrella reshapes argv so the
group's own argparse sees a clean program name. The eight legacy console-scripts
...
from .cli import capabilities, concat, image, job, speech, usage, video
