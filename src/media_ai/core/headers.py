...
* **It may not be the credential, and it may not be framing.** The key is named by the
  binding (``credential = "env://…"``), and ``Content-Type``/``Content-Length``/``Host``
  are decided per request by the transport. Either would be *silently* lost under the
  headers the adapter sets, and a setting that quietly does nothing is worse than a
  refusal.
...
__all__ = ["RESERVED_HEADERS", "parse_headers", "split_header_argument"]
