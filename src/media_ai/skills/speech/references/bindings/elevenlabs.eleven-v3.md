...
> Parameters and limits: `{{cli}} capabilities --binding elevenlabs/eleven-v3`.
...
```bash
{{cli}} speech dialogue --binding elevenlabs/eleven-v3 \
...
- Cost is per character across the whole script, cast list included where it is
  spoken. Long dialogues are the expensive case here — check `{{cli}} usage` after a
