...
```toml
# ~/.config/{{cli}}/config.toml — non-secret, safe to share
...
| `cred://<account>` | a `[<account>]` block in `~/.config/{{cli}}/credentials.toml` (chmod 600) |
...
`{{cli}} bindings add <id> --credential env://VAR` writes the reference only.
`{{cli}} init` also offers to store the key itself in `credentials.toml`
