...
{"ok": true, "bindings": [
 ...
 "config": "/home/you/.config/media-ai/config.toml"}
```
...
- `defaults` is what runs when a command names no binding — this is why a bare
  `media-ai video generate` works at all.
...
{"bindings": [{"binding": "gemini/veo-3.1", "scenes": ["video.extend", "…"],
               ...
               "add": "media-ai bindings add gemini/veo-3.1 --credential env://GEMINI_API_KEY"}]}
```
