...
name: {{skill}}music
description: >-
  Compose music from a text prompt or a structured composition plan via the {{cli}}
...
metadata:
  requires:
    bins: ["{{cli}}"]
  cliHelp: "{{cli}} capabilities --scene music.text_to_music"
  install:
    tier: optional
    summary: >-
...
```bash
{{cli}} capabilities --scene music.text_to_music --configured   # who can serve this?
{{cli}} capabilities --binding <id> --pretty                    # what does it accept?
```
...
{{cli}} music generate --plan song_plan.json --seed 42 --detailed true --output song.mp3
```
