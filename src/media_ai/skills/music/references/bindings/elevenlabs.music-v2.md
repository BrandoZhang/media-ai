...
> Parameters and limits: `{{cli}} capabilities --binding elevenlabs/music-v2`.
...
```bash
{{cli}} music plan --prompt "upbeat indie-pop, verse/chorus/bridge, ~2 min" \
    --duration-ms 120000 --output plan.json
# edit plan.json — section names, durations, per-section styling
{{cli}} music generate --plan plan.json --seed 42 --output song.mp3
```
...
- Cost is metered in the `speech_characters` ledger field like other audio ops, so a
  batch of long tracks shows up where you might not look for it — check `{{cli}} usage`.
