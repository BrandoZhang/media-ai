...
name: {{skill}}sound
description: >-
  Generate a sound effect from a text description via the {{cli}} CLI. Covers
...
metadata:
  requires:
    bins: ["{{cli}}"]
  cliHelp: "{{cli}} capabilities --scene sound.text_to_sound"
  install:
    tier: optional
    summary: >-
...
One scene: **`{{cli}} sound generate`** (`sound.text_to_sound`) — `--text` describing
the effect → an audio file.

## Discover first

```bash
{{cli}} capabilities --scene sound.text_to_sound --configured
{{cli}} capabilities --binding <id> --pretty
```
