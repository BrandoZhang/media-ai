...
metadata:
  requires:
    bins: ["media-ai"]
  cliHelp: "media-ai capabilities --scene animation.from_video"
  install:
    tier: optional
    summary: >-
...
# media-ai-animation — clip or stills → animated image
...
One command, **`media-ai animation export`**, serving two scenes. As everywhere, **what
...
```bash
media-ai capabilities --scene animation.from_video --configured
media-ai capabilities --binding local/ffmpeg --pretty
```
...
media-ai animation export --input demo.mp4 --output demo.webp --max-width 640 --fps 12

# a 3-second GIF from the middle of a longer clip, played twice
media-ai animation export --input talk.mp4 --output clip.gif \
    --start 12 --duration 3 --max-width 480 --fps 10 --loop 2

# a seamless loop out of a clip that does not loop
media-ai animation export --input wave.mp4 --output wave.webp --bounce --max-width 400
...
media-ai animation export --frames frames/ --output turntable.webp --fps 24
```
...
- Cost is zero and nothing leaves the machine, but the call is still recorded in the
  ledger under `local/ffmpeg` — see `media-ai-usage`.
