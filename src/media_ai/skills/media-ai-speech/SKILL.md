...
description: >-
  Turn text into spoken audio via the media-ai CLI — single-voice text-to-speech and
...
metadata:
  requires:
    bins: ["media-ai"]
  cliHelp: "media-ai capabilities --scene speech.text_to_speech"
  install:
    tier: optional
    summary: >-
...
|---|---|---|
| `media-ai speech generate` | `speech.text_to_speech` | one voice reads `--text` |
| `media-ai speech dialogue` | `speech.dialogue` | several voices act out a script |

**A binding that serves one need not serve the other.** Ask which do:

```bash
media-ai capabilities --scene speech.dialogue --configured   # who can act out a script?
...
media-ai speech generate --text "Welcome aboard. Please fasten your seatbelt." \
    --output welcome.mp3

# A specific binding, a specific voice, tuned with its own option keys
media-ai speech generate --binding <provider>/<model> \
...
- `--timestamps true` adds a **second artifact**; dialogue sidecars also carry
  `voice_segments`. Cost is metered in `speech_characters` — see `media-ai-usage`.
