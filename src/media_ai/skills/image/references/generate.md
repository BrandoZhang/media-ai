# `{{cli}} image generate` / `image edit` — full reference
...
`../../{{skill}}shared/references/machine-contract.md`.
...
```bash
{{cli}} image generate --prompt "..." --output out.png --metadata-out meta.json
...
adjust to what `{{cli}} capabilities` reports, or pass `--on-unsupported warn` to
