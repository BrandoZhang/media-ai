...
description: >-
  Discover what the {{cli}} CLI can actually do on this machine — which bindings
  ...
  BEFORE requesting a generation. Use when asked which {{cli}} models or providers
...
# {{skill}}capabilities — discover before you generate

> Read `../{{skill}}shared/SKILL.md` for the machine contract. This is the
> **discovery entry point** every generation skill should call first.

`{{cli}} capabilities` prints the binding manifests. Discovery and pre-flight
...
```bash
{{cli}} capabilities --pretty                        # every binding this package knows
{{cli}} capabilities --configured                    # only the ones reachable right now
{{cli}} capabilities --scene video.image_to_video    # only the ones serving this scene
{{cli}} capabilities --binding <provider>/<model> --pretty     # one, in full
```
...
- **`scenes[]`** — what this binding serves. The CLI derives the scene from your
  inputs (see `../{{skill}}shared/SKILL.md`); a binding that does not declare it is
