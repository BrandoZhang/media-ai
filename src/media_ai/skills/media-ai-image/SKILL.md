...
description: >-
  ...
  the media-ai CLI. Handles geometry (pixel size or aspect-ratio + resolution tier),
...
metadata:
  requires:
    bins: ["media-ai"]
  cliHelp: "media-ai capabilities --scene image.text_to_image"
  install:
    tier: optional
    summary: >-
...
No flag selects the scene — the references do. `media-ai image edit` is the same
...
```bash
media-ai bindings list                                # what can this machine call?
media-ai capabilities --scene image.image_to_image    # who serves this scene?
...
Plus the global flags from `media-ai-shared`: `--binding` / `--provider` / `--model`,
...
media-ai image generate --prompt "a red bicycle on a white seamless background" \
    --output bike.png

# Same, on a specific binding with its own geometry and options
media-ai image generate --binding <provider>/<model> \
