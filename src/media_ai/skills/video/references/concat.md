...
`{{cli}} video concat` runs the bundled ffmpeg on this machine: **free, offline, no
...
{{cli}} video concat --inputs a.mp4 b.mp4 --output film.mp4 --width 1280 --height 720
```
...
listed by `{{cli}} bindings list`, always available because there is nothing to
configure.

## When it is the wrong tool

- **An animated GIF / WebP / APNG** — that is `{{cli}} animation export`
  (the `{{skill}}animation` skill), which also covers trimming, frame rate, speed,
