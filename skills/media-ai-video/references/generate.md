...
| `--last-frame PATH` | none | ending still (`role=last_frame`); needs `supports_last_frame` |
...
| `--seed N` | none | where `supports_seed` |
...
| `--negative-prompt TEXT` | none | where `supports_negative_prompt` (e.g. Veo) |
