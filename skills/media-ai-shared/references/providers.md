...
| `openai` | ✓ GPT-Image / DALL·E | ⚗️ Sora (experimental) | yes | ✓ | `OPENAI_API_KEY` |
...
- Else **inferred from `--model`** by hint: `doubao*`/`seedream*`/`seedance*` ⇒ `volc`;
  `gpt-image*`/`dall-e*`/`sora*` ⇒ `openai`; `gemini-*`/`veo-*` ⇒ `gemini`.
...
`gpt-image-1`, `gpt-image-1-mini`, `dall-e-3`, `dall-e-2`, `sora-2`, `sora-2-pro`;
...
| `openai` | `OPENAI_BASE_URL` (`https://api.openai.com/v1`) | `OPENAI_IMAGE_MODEL`, `OPENAI_VIDEO_MODEL` | `OPENAI_POLL_INTERVAL` (5s), `OPENAI_POLL_TIMEOUT` (900s) |
